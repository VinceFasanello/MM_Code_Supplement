...
Analysis: Power Quick Look (NEW NEW NEW)
```{r}
...
my <- myfa[order(myfa$bctID),]
mymod <- lm(my$deltafit ~ my$bctID, weights = my$rdeltafit); summary(mymod)
...
my <- myfa[order(myfa$bctID),]
mymod <- lm(my$fit25 ~ my$bctID, weights = my$rfit25); summary(mymod)
...
p <- ggplot(my, aes(x=myx, y=myy, weight = myr, color = myc)) +
  ...
  theme(axis.text.y = element_text(size = 8, angle = 0))+
  theme(axis.text.x = element_text(size = 8, angle = 0))+
  annotate("text", x=c(2,3,4,5.25,6), y=c(.065,.25,.045,.09,.115), label = c("***", "***", "*", ".", "***"), size = 8, fontface = "bold")
